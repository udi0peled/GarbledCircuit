...
void init_circuit_info() {

  for (int i = 0; i < 3; ++i) {

    circuit_info.num_inputs[i] = 0;
    circuit_info.num_outputs[i] = 0;

    free(circuit_info.inputs[i]);
    free(circuit_info.outputs[i]);

    circuit_info.inputs[i] = NULL;
    circuit_info.outputs[i] = NULL;
  }

  circuit_info.gate_count = 0;
  circuit_info.num_not_gates = 0;
  circuit_info.num_and_gates = 0;
  circuit_info.num_xor_gates = 0;
  circuit_info.num_or_gates = 0;

  free(circuit_info.circuit);
  circuit_info.circuit = NULL;
}

void finish_circuit_info() {
  init_circuit_info();
}
...
void add_gate_info(OblivBit* r, const OblivBit* a, const OblivBit* b, char ttable) { 
  ...
  if ((ttable & 0x0f) == 0x0f) {
    ...
    new_gate.input1 = new_index;
    new_gate.input2 = new_index;
    
  } else {

    new_gate.input1 = a->index;
    new_gate.input2 = b->index;
  }

  // Only relevant to compute here for non inputs
  unsigned char input1_val;
  unsigned char input2_val;

  if (ttable != 0x0f) {
    input1_val = circuit_info.circuit[a->index].ttable & 0x10;
    input2_val = circuit_info.circuit[b->index].ttable & 0x10;
  }

  unsigned char result_val = 0x00;

  switch (ttable & 0x0f) {
    
    case 0x03: 
      circuit_info.num_not_gates++;
      result_val = ~input1_val;
      break;
    
    case 0x06: 
      circuit_info.num_xor_gates++;
      result_val = input1_val ^ input2_val;
      break;
    
    case 0x08: 
      circuit_info.num_and_gates++;
      result_val = input1_val & input2_val;
      break;
    
    case 0x0e: 
      circuit_info.num_or_gates++;
      result_val = input1_val | input2_val;
      break;
    ...
    default:
      fprintf(stderr, "unknown gate ttable %x at index %lu\n", ttable, new_index);
      exit(1);
      break;
  }

  new_gate.ttable = ttable;

  if (should_evaluate_circuit) {
    new_gate.ttable |= result_val & 0x10;
  }

  //memcpy(new_gate->key, r->yao.w, sizeof(yao_key_t));

  circuit_info.circuit[new_index] = new_gate;
...
void add_input_gate(OblivBit* gate, int party) {
  if ((0 <= party) && (party < 3)) {
    ...
    if (should_evaluate_circuit) {
      circuit_info.circuit[gate->index].ttable |= (gate->knownValue & 0x01) << 4;
    }
  }
}
...
int save_circuit_to_file() {

  FILE *outfile; 
      
  // open file for writing 
  outfile = fopen ("/tmp/gc.bin", "wb"); 
  if (outfile == NULL) 
  { 
      fprintf(stderr, "\nError opening file /tmp/gc.bin\n"); 
      return 1;
  }
...
static void dbgFeedOblivBool(OblivBit* dest,int party,bool a)
{
  ...
  add_gate_info(dest, dest, dest, 0x0f);
  add_input_gate(dest, party);
}
...
bool dbgProtoRevealOblivBits(ProtocolDesc* pd,widest_t* dest,const OblivBit* src,size_t size,int party)
{ 
  for (int i = 0; i < size; ++i) {
    add_output_gate(&src[i], party);
  }
                     ...
                     protocol_run start, void* arg)
{
  ...
  if(me==1)
  {
    ...
    memset(ypd->R, 0xff, YAO_KEY_BYTES);
    memset(ypd->I, 0xff, YAO_KEY_BYTES);
